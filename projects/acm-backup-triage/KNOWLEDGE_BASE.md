...
**Key points:**
  ...
  - **Failback scenario (round-trip failover/failback):** MSA auto-import will ALWAYS fail on failback for imported clusters. When the spoke connected to the passive hub during failover, the passive hub recreated the `auto-import-account` ServiceAccount on the spoke (new UID). The token in the backup references the old SA UID — Kubernetes rejects it with "Unauthorized" even though the JWT hasn't expired. This is a known limitation.
    ...
    - **New diagnostic heuristic (from occurrence #2):** if only some managed clusters fail auto-import on failback while others succeed, check whether the failing ones are *imported* vs the succeeding ones being *Hive-created* (`oc get clusterdeployment -n <cluster-ns>` — presence confirms Hive). Hive-created clusters reconnect via their ClusterDeployment admin kubeconfig and never hit this MSA limitation. Cloud platform can be a tell: non-OpenShift platforms (AKS, EKS, GKE) can only ever be *imported* (Hive only provisions OpenShift), so they are the most likely to exhibit this failure when mixed with Hive-provisioned OpenShift clusters (e.g., ARO) in the same fleet.
